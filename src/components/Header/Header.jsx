...
const Header = () => {
  const [isOpen, setIsOpen] = useState(false); // Language menu
  const [menu, setMenu] = useState(false); // Main menu
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setIsLoading(false);
    }, 3000); // Set to your desired loading duration
  }, []);
  ...
  return (
    <>
      {/* Loading Spinner */}
      {isLoading && (
        <div className="fixed top-0 left-0 w-screen h-screen bg-[#00000080] z-50 flex items-center justify-center">
          <ClipLoader size={100} color="#ffffff" loading={isLoading} />
        </div>
      )}
        ...
        <div className={`${Mode ? "bg-white" : "bg-black"} pt-5`}>
          <ul className="flex flex-col mb-7">
            ...
            <li className={`py-[15px] ${Mode ? "text-black" : "text-white-person"}`}>
              <NavLink onClick={handleMenuToggle} to={"/sessions"} className="flex justify-center">
                <p className="flex gap-1 items-center">
                  <MdOutlineTheaterComedy className="w-6 h-6" /> <span>Сеансы</span>
                </p>
              </NavLink>
            </li>
            <li className={`py-[15px] ${Mode ? "text-black" : "text-white-person"}`}>
          ...
          <div className="flex justify-center pb-10 h-14 z-50 mx-auto relative">
            ...
            >
              ...
              <span className={`${Mode ? "text-black" : "text-white-person"} font-aeonik`}>
                {selectedLanguage}
              </span>
              <MdKeyboardArrowDown
            ...
            {isOpen && (
              <div
                className={`absolute top-full ${Mode ? "bg-black text-white" : "bg-white text-black"} mt-2 border border-gray-700 rounded-md py-1 min-w-[100px]`}
              >
